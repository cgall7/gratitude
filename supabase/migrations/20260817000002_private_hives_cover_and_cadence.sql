...
--
-- Five theme ids, matching GUIDES/PRIVATE_HIVE_DESIGN_LANGUAGE.md §1 exactly
-- (golden-honey is the spec's default/hero theme). PLANS/PRIVATE_HIVES_COVER_THEMES.md
-- names a different six-theme set -- that file predates the design-language
-- doc Deezine posted as "ready for implementation" today and is superseded
-- by it; not used here.
alter table public.private_hives
  add column cover_theme text not null default 'golden-honey',
  add column review_cadence text not null default 'yearly';

alter table public.private_hives
  add constraint private_hives_cover_theme_check
    check (cover_theme in ('golden-honey', 'sunlit-honey', 'wildflower', 'starlight', 'cream-gold'));
