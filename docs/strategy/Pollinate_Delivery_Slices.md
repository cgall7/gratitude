...
>
> **Open item — send-gate cardinality (flagged 2026-08-17, Sage):** `sealed_at` (`20260815000004`, shipped) and `sent_at` (point 2 above, specified) share the same one-directional guard — null → timestamp, once, ever; no unseal/unsend flow. So per hive, every tier gets ≤1 seal and ≤1 send, permanently. A per-hive-per-year send figure can't distinguish free from paid on that basis alone; it's constant either way. **Pending Colin:** is the free tier's "1 hive" *concurrent* (replace it after review; many hives over a lifetime, one live at a time) or *lifetime* (one hive, ever)? Concurrent → the only real paywall is an account-level send count, which needs `send_hive` as its counter. Lifetime → hives is the sole live counter and the send row is moot. Pricing rows in the PRD and Strategy docs carry a "pending" placeholder until this lands.
>
...
> 4. **The send act is one SECURITY DEFINER RPC** (`send_hive(hive_id)` shape, not yet in schema; house revoke pattern per `20260813000005`): validates caller = owner, `sealed_at` set, `subject_profile_id` non-null, an accepted connection — then flips the hive's `packaged` entries to `'sent'` and sets `sent_at` in one transaction. Two client-side writes would allow `sent_at` without flipped entries: the recipient opens an empty reveal, the exact failure Slice 1 exists to avoid. The RPC is also the single choke point for pricing: **ruled by Colin 2026-08-17 — pricing gates the send, never the seal.** Sealing stays free and unconditional; any future paywall check lives inside `send_hive`, nowhere else.
...
| 12.3 | Freemium free tier               | New users automatically get free tier. No trial period, no card required. Free tier: 1 hive, 1 friend, 1 seed, daily journal (full), yearly review only, send gate *(2026-08-17: pending Colin's ruling — `sealed_at`/`sent_at` cap every tier at ≤1 send/hive forever, so a per-hive-per-year figure is vacuous as written; see 8b.5 open item)*, full feed access, receive unlimited. | M   | ios, android, backend |
...
> - **Free tier:** Receive unlimited, 1 hive, 1 friend, 1 seed, daily journal (full), yearly review only, send gate pending Colin's ruling (sealing itself is unconditional, 2026-08-17 — see 12.3), full feed access.
> - **Paid tier (Pollinate Plus):** Unlimited hives, friends, seeds, monthly/yearly/manual reviews, send gate pending ruling, Cash App gifting, premium themes.
